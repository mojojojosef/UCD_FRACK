...
public class Cell : MonoBehaviour{
	...
	void OnMouseUp() {
		if(MasterControl.control.currGame.currentTurn >= readyTurn)
		{
			...
			{
				//Survey/Buy
				if(MasterControl.control.currGame.players[MasterControl.control.currGame.currentPlayer].money >= currentCost)
				{
					owner = MasterControl.control.currGame.currentPlayer;
					MasterControl.control.currGame.players[MasterControl.control.currGame.currentPlayer].playerOwnedCells[MasterControl.control.currGame.players[MasterControl.control.currGame.currentPlayer].cellsOwned++] = this;
					MasterControl.control.currGame.players[MasterControl.control.currGame.currentPlayer].money -= currentCost;
			...
			else
			{
				if(owner == MasterControl.control.currGame.currentPlayer && !isEmpty)
				{
					switch(phase)
					{
						case 1:
						{
							if(MasterControl.control.currGame.players[owner].money >= currentCost)
							{
								GetComponent<Renderer>().material.SetTextureOffset("_MainTex", new Vector2(0.375f, 0.0f));
								MasterControl.control.currGame.players[owner].money -= currentCost;
								MasterControl.control.currGame.UpdateMainUI();
